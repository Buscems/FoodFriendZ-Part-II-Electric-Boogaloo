    ...
    void FixedUpdate()
    {
        if (!playerFalling)
        {
            ...
            if (Mathf.Abs(distance) >= radius)
            {
                ...
                if (!bossCamera)
                {
                    transform.position = Vector3.Slerp(currentPos, playerPos, player.transform.parent.transform.parent.GetComponent<MainPlayer>().speed * cameraSpeed * Time.fixedDeltaTime);
                }
                else
                {
